...
/*
    Implement the graph using adjacency lists and analize it using Dijkstra's algorithm for shortest path.
    by JL
    25.02.2021
*/
...
void Test_Graph()
{
    ...
    graph.AddEdge(1,4,10); // B <-> E
    graph.AddEdge(1,3,5);  // B <-> D
    graph.AddEdge(2,3,5);  // C <-> D
    graph.AddEdge(2,4,1);  // C <-> E
    graph.AddEdge(3,4,4);  // D <-> E
    graph.AddEdge(3,4,1);  // C <-> E
    graph.AddEdge(3,4,4);

    std::cout<<"\n------\nTITLE: Creating the graph using weighted adjacency lists and analizing it using Dijkstra's algorithm for the shortest path.\n\n";
    graph.PrintEdges();
